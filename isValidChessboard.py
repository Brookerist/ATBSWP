...
def isValidChessboard(userPiece):
    pieces = validBoard.values()  # variables to reduce complexity of expressions
    spaces = validBoard.keys()
    if userPiece in pieces:  # checks pieces against list of legal pieces
        print('valid piece')
        print('Move the ' + pieceIndex[userPiece] + ' to what position?')
        userMove = input()
        userMoveFrom = list(spaces)[list(pieces).index(userPiece)]  # seems to be the right way to pull the space that the piece is coming from
        if userMove in spaces:  # checks position against board area
            print('position inside board bounds')
            if validBoard[userMove] == '   ':  # checks to ensure empty space for move.
                print('valid move')
                validBoard[userMove] = userPiece  # move piece to new space and erase old space
                validBoard[userMoveFrom] = '   '
            else:
                print('there is a piece there already')
        else:
            print('position selected is outside of the board area')
    else:
        print('not a recognized piece')
...
# todo:
# ownership of pieces black/white
# white/black turn flip/flop on successful move
# piece movement behavior
# make pieces "takeable"
